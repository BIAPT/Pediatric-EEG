%% modified pipeline for Pediatric Data_SCT_baseline

%% Define Parameters
% wPLI and dPLI parameters
frequencies = ["alpha","theta","delta"]; % This can be ["alpha" "theta" "delta"];
...
thresholds = [0,0,0]; % array to save SCT threshold values of 3 frequency bands
...
for f = 1:numel(files)
   ...
   % outdir_topographicmap = fullfile(resultsfolder, ID, "Whole",'Topographic Maps');
        ...
        for h = 1:length(hemispheres)
            ...
            if hemisphere == "Whole"
                ...
                if threshold == "SCT"
                    ...
                    thresholds(fr)=finalthreshold; % save the threshold for baseline state at frequency band
    ...
    % Save threshold values of baseline state in 3 frequency bands
    thresholdresults_dir = fullfile(resultsfolder , ID,'threshold_results.txt');
    fileID = fopen(thresholdresults_dir,'w');
    fprintf(fileID,'%0.2f\n',thresholds); % each threshold value saved in row
    fclose(fileID);
    disp("Threshold values saved")
